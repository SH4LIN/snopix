...
Pixel Scout adds reverse-image search to the WordPress media library. Upload
...
* Reverse-image search via the admin dashboard or a `[ps_search]` shortcode for
  the front end.
...
* REST API at `/wp-json/ps/v1/` with rate limiting on the public search
  endpoint.
...
Pixel Scout creates a single custom table, `{prefix}ps_index`, that stores one
row per indexed attachment with its three fingerprints. Uninstalling the
plugin drops the table.

== Installation ==

1. Upload the `pixel-scout` folder to `/wp-content/plugins/` (or install the
   zip via **Plugins → Add New → Upload Plugin**).
2. Activate the plugin in **Plugins**.
3. Go to **Media → Pixel Scout** and click **Index Remaining** to fingerprint
...
uninstall hook drops `{prefix}ps_index` and removes all plugin options and
transients.
...
* Reverse-image search via admin dropzone and `[ps_search]` shortcode.
